-- ==============================================================================
-- TennisPlay - Migração 005: Bloqueio de Reservas em Datas e Horários Passados
-- Arquivo: supabase/migrations/005_bloquear_reservas_passadas.sql
-- ==============================================================================

BEGIN;

-- 1. Atualizar RPC public.criar_reserva com validação de data civil e horário
...
BEGIN
  ...
  -- 3. Validar vínculo do usuário com o grupo
  ...
  -- 4. Obter e validar a quadra do grupo
  ...
  -- 5. Obter e validar o horário do grupo
  ...
  -- 7. Verificar conflito de reserva existente (concorrência e duplicidade)
  IF EXISTS (
  ...
  -- 8. Inserir a reserva atomicamente vinculando a classe real do membro no grupo
  BEGIN
  ...
  -- 9. Carregar dados do usuário criador para resposta
...
-- 2. Trigger de segurança a nível de tabela para impedir INSERTs em datas passadas (mesmo via REST direto)
CREATE OR REPLACE FUNCTION public.validar_reserva_data_futura()
RETURNS TRIGGER AS $$
DECLARE
...
DROP TRIGGER IF EXISTS trg_validar_reserva_data_futura ON public.reservas;
CREATE TRIGGER trg_validar_reserva_data_futura
  BEFORE INSERT OR UPDATE OF data ON public.reservas
  FOR EACH ROW
  EXECUTE FUNCTION public.validar_reserva_data_futura();
